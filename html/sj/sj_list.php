...
<head>
	...
	<link rel="styleshv eet" href="font.css">
</head>
<body>
...
<table width="400" border="1" cellpadding="2" style="border-collapse:collapse">
  ...
  <?
  	if (!$text1)
  	$query="select * from sj order by name8;";
	else
	$query="select * from sj where name8 like '%$text1%' order by name8;";
	...
	for($i=0; $i<$page_last; $i++)
	{
		...
		$avg=sprintf("%6.1f",$row[avg8]);
			...
			<td align='center'>&nbsp
					<a href='sj_edit.php?no=$row[no8]'> $row[name8]</a></td>
					<td width='50'  align='right'>$row[kor8]</td>
					<td width='50'  align='right'>$row[eng8]</td>
					<td width='50'  align='right'>$row[mat8]</td>
					<td width='50'  align='right'>$row[hap8]</td>
					<td width='50'  align='right'>$row[avg8]</td>
					
						<td align='center'>
						<a href='sj_delete.php?no=$row[no8]'
